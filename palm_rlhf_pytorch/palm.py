...
from palm_rlhf_pytorch.utils import (
    top_p,
    top_k,
    masked_mean,
    gumbel_sample,
...
class PaLM(nn.Module):
    ...
    def add_finetune_params(self, scope, lora_r=None):
        assert (
            scope not in self.finetune_modules
        ), f"finetune scope {scope} already found"
        dim, dim_head, heads, r, device = (
            self.dim,
            self.dim_head,
            self.heads,
            default(lora_r, self.lora_r),
            self.device,
        )
    ...
    def merge_finetune_params(self, scope):
        ...
        for layer, (lora_q, lora_k, lora_v, lora_o) in zip(self.layers, lora_modules):
            ...
            lora_qkv_weight = F.pad(
                lora_qkv_weight, (0, fused_proj_out_dim - lora_qkv_weight.shape[1])
            )
    ...
    def finetune_parameters(self, scope="default"):
        assert (
            scope in self.finetune_modules
        ), f"finetune parameters of scope {scope} not found"
    ...
    def forward(
        ...
        return F.cross_entropy(
            logits, labels, ignore_index=self.cross_entropy_ignore_index
        )
