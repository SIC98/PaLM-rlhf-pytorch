...
class ActorCritic(nn.Module):
    def __init__(
        ...
        if self.critic_lora:
            self.critic_palm.add_finetune_params(
                critic_lora_scope, lora_r=critic_lora_r
            )

        self.pooled_values = pooled_values
        self.value_head = nn.Sequential(
            nn.Linear(palm.dim, 1), Rearrange("... 1 -> ...")
        )
    ...
    @eval_decorator
    def generate(
        self, state, max_seq_len, eos_token=None, return_values=False, **kwargs
    ):
        ...
        action_logits, value = self.forward(
            sequence, mask=action_mask, return_values=return_values
        )
    ...
    def forward(self, x, mask=None, return_values=True):
        action_logits = self.actor_palm(x, finetune_scope=self.actor_lora_scope)

        if not return_values:
            return action_logits, None

        critic_embeds = self.critic_palm(
            x, return_only_embedding=True, finetune_scope=self.critic_lora_scope
        )
...
def log_prob(prob, indices):
    assert (
        prob.shape[:2] == indices.shape
    ), f"preceding shapes of prob {prob.shape[:2]} and indices {indices.shape} must match"
...
class RLHFTrainer(nn.Module):
    def __init__(
        ...
        *,
        ...
        if exists(prompts):
            ...
            assert exists(
                tokenizer
            ), "tokenizer must be passed in if raw text prompts are given"
    ...
    def generate(
        ...
        **kwargs,
        ...
        (
            ...
            prompt, *args, max_seq_len=max_seq_len, return_values=False, **kwargs
        )

        rewards = reward_model(
            sequences, prompt_mask=prompt_mask, mask=mask, sample=True
        )
    ...
    def learn(self, memories: Deque[Memory]):
        # stack all data stored in the memories

        all_memories_stacked_and_padded = list(
            map(partial(pad_sequence_fixed, batch_first=True), zip(*memories))
        )

        # prepare dataloader for policy phase training

        dl = create_dataloader(
            all_memories_stacked_and_padded, self.minibatch_size, device=self.device
        )
        ...
        for _ in range(self.epochs):
            for (
                ...
                if self.kl_div_loss_weight > 0:
                    kl_penalty = (
                        masked_kl_div(old_action_probs, action_probs, mask=action_masks)
                ...
                if old_values.ndim == 2:
                    old_values, values = map(
                        lambda t: shift(t, shift=1, dim=-2), (old_values, values)
                    )
                ...
                if exists(self.max_norm):
                    self.accelerator.clip_grad_norm_(
                        self.actor_critic.actor_parameters(), self.max_norm
                    )
                ...
                if exists(self.max_norm):
                    self.accelerator.clip_grad_norm_(
                        self.actor_critic.critic_parameters(), self.max_norm
                    )
    ...
    def train(
        ...
        for eps in tqdm(range(num_episodes), desc="episodes"):
            for timestep in range(max_timesteps):
                ...
                prompt_mask = (
                    torch.arange(sequence.shape[-1], device=device) < prompt_length
                )
                ...
                mask = default(
                    mask,
                    lambda: torch.ones(sequence.shape, dtype=torch.bool, device=device),
                )

                reward = self.reward_model(
                    sequence, prompt_mask=prompt_mask, mask=mask, sample=True
                )
